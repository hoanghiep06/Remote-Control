...
DLLEXPORT void CaptureScreenToJpeg() {
    ...
    // Dùng command TAKEPIC/TAKE giống CaptureScreen cũ
    sendCommandInternal("TAKEPIC");
    Sleep(500);
    sendCommandInternal("TAKE");

    // Nhận dữ liệu ảnh BMP vào file tạm
    receiveImageResponseInternal(); // ghi screenshot.bmp
    sendCommandInternal("QUIT");

    // [TÙY CHỌN] Nếu bạn có dùng OpenCV / stb_image để convert BMP → JPG, xử lý ở đây
    // Ví dụ: dùng OpenCV (nếu link với OpenCV):
    //
    // cv::Mat img = cv::imread("screenshot.bmp");
    // if (!img.empty()) {
    //     cv::imwrite("screen_current.jpg", img);
    // }
    //
    // Còn nếu không, cứ dùng screenshot.bmp cũng được, Python/webapp đọc file đó luôn.
}

DLLEXPORT void ReceiveScreenStream() {
    if (clientSocket == INVALID_SOCKET) return;

    sendCommandInternal("SCREEN");
    Sleep(200);
    sendCommandInternal("START");

    const int MAX_BUFFER = 2 * 1024 * 1024;
    char* buffer = new char[MAX_BUFFER];
    ...
    while (true) {
        ...
        // Ghi đè vào webcam.jpg (Web đọc file này)
...
DLLEXPORT void RecordScreenStream(int durationSeconds, const char* folderPath) {
    ...
    // Gửi lệnh sang server: SCREEN_REC (ví dụ) – bạn phải implement bên server cho khớp
    ...
    if (durationSeconds > 0) {
        start += " " + to_string(durationSeconds);
    }
    sendCommandInternal(start);
    ...
    while (true) {
        string header = receiveLine();
        if (header == "STOPPED" || header == "TIMEOUT" || header == "QUIT" || header.empty())
            break;
        ...
        // Tạo tên file: folderPath\screen_00001.jpg ...
        std::ostringstream oss;
        oss << folderPath;
        if (folderPath[strlen(folderPath) - 1] != '\\' && folderPath[strlen(folderPath) - 1] != '/')
            oss << "\\";
        oss << "screen_" << std::setw(5) << std::setfill('0') << frameIndex++ << ".jpg";
...
DLLEXPORT void ReceiveStreamGeneric(const char* modeCmd, const char* outFilename) {
    if (clientSocket == INVALID_SOCKET) return;

    // 1. Gửi lệnh chọn chế độ (Linh hoạt)
    sendCommandInternal(modeCmd); 
    Sleep(200); // Chờ server chuyển mode

    // 2. Gửi lệnh BẮT ĐẦU
    sendCommandInternal("START");

    // Tăng buffer lên 2MB để chịu được cả ảnh màn hình HD
    const int MAX_BUFFER = 2 * 1024 * 1024; 
    char* buffer = new char[MAX_BUFFER];
    ...
    while (true) {
        ...
        // 3. Ghi ra file theo tham số truyền vào
...
// --- RESTART (Khởi động lại máy Server) ---
DLLEXPORT void RestartServer() {
    if (clientSocket == INVALID_SOCKET) return;
    sendCommandInternal("RESTART");
}
